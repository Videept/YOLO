...
public class ConfigViaSocket {
    ...
    public interface RecogniseByteArray {
        Detections recognise(byte[] b, int w, int h, int rotation);
    }
    ...
    private class SocketServerThread extends Thread {
        ...
        private yuv_image readJPEG(int image_size,InputStream in,PrintStream out) {
            ...
            Bitmap b = BitmapFactory.decodeByteArray(bb, 0, image_size); // decodes jpeg to RGB bitmap
                    ...
                    while (input!=null) {
                        ...
                        try {
                            if (parts[0].equals("SET")) {
                                ...
                                Detections detects = d.recognise(yuv.yuv.array(), yuv.width, yuv.height, 0);
                                ...
                                for (i = 0; i < num_images; i++) {
                                    ...
                                    Detections detects = d.recognise(yuvlist.get(i).yuv.array(), yuvlist.get(i).width, yuvlist.get(i).height, 0);
                                    Logger.addln("done");
