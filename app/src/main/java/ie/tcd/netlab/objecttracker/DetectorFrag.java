...
public class DetectorFrag extends CameraPlusOverlayFrag {
    ...
    public void resumeDetectorFrag() {
        ...
        if (!useSOCKET && configViaSocket != null) {
            configViaSocket.onDestroy();
            configViaSocket = null;
    ...
    public void processImage(final Image image) {
        ...
        //bb.get(data);
        // byte array to mat
        //Mat bjp = Imgcodecs.imdecode(new MatOfByte(data), Imgcodecs.CV_LOAD_IMAGE_UNCHANGED);
        ...
        /**KARAN'S CODE
         ...
         // byte array to mat
         //Mat bjp1 = Imgcodecs.imdecode( new MatOfByte(data), Imgcodecs.CV_LOAD_IMAGE_UNCHANGED);
         Mat diff = new Mat();
         Core.absdiff(imageGrab,prev,diff); //absdiff( frame, background, difference );
         prev=imageGrab;

         //Abhinav - Now convert the difference image to image format
         image = diff; **/
    ...
    class ConfigViaSocketRecognise implements ConfigViaSocket.RecogniseByteArray {
        public Detections recognise(final byte[] yuv, int w, int h, int rotation) {
            // take YUV byte array as input for processing.  used for testing.
            return detector.recognize(yuv, w, h, rotation);
        }
    }
    ...
    public void updateOverlay(final Canvas canvas) {
        ...
        if ((detects == null) || (detects.results == null) || (detects.results.size() == 0))
